...
def _secure_file(path: Path) -> None:
    """Set file permissions to user-only (Unix) or rely on Windows default permissions."""
    if platform.system() != 'Windows':
        os.chmod(path, 0o600)
    # On Windows, files in user's home directory are already only accessible to current user
...
class TokenManager:
    ...
    def load_token(self, provider: str) -> Optional[str]:
        """Load and decrypt API token from disk, or None if not found."""
        token_file = CONFIG_DIR / f"{provider}_token.enc"
...
class AssemblyPreprocessor:
    ...
    # NOTE: These patterns are MSVC-specific. Code compiled with MinGW, Clang,
    # or other compilers will have different debug patterns and may not be detected.
    DEBUG_PATTERNS = [
        r'__\$EncStackInit.*',
...
def validate_and_report_preprocessing(original_lines: int, processed_lines: int,
                                      removed_lines: list, api_calls: list) -> bool:
    """Validate preprocessing didn't remove critical code and report warnings."""
    warnings = []
  ...
  %(prog)s --list-providers
    ...
    try:
        ...
        try:
            ...
            print(f"✗ Error: Failed to read file with alternative encoding: {e}")
            return 1
        ...
        print(f"✗ Error: File too large to read into memory")
        return 1
    except Exception as e:
    ...
    if not args.preserve_all:
        validate_and_report_preprocessing(
            stats['code_lines'],
